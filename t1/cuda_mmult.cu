...
// perform the computation on the CPU instead on the device
//#define COMP_CPU
...
int main(int argc, const char *argv[]) {
   ...
   float *Ad, *Bd, *Cd;
   // DONE: Allocate space on the GPU 
   ...
   // DONE: Copy matrix data to device   
...
#ifndef COMP_CPU
   // DONE: Copy result back to host
   cudaMemcpy(C, Cd, size, cudaMemcpyDeviceToHost); checkCUDAError("copy back to host for C");
   // DONE: Free device memory
   cudaFree(Ad);
   cudaFree(Bd);
   cudaFree(Cd);
#endif
